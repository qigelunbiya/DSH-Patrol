...
function snapshotTitleActionCollect() {
  ...
  const pathSelector = element => {
    ...
    if (title) {
      ...
      if (element.closest?.('.ant-tree,[role="tree"]')) {
        let ancestor = element.parentElement
        for (let depth = 0; ancestor instanceof Element && depth < 7; depth += 1, ancestor = ancestor.parentElement) {
          for (const className of [...ancestor.classList]) {
            if (!/^[A-Za-z_][A-Za-z0-9_-]*$/.test(className) || /^ant-/.test(className) || className === 'tree') continue
            const candidate = `.${cssEscape(className)} ${byTitle}`
