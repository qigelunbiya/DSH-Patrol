...
export function apply(ctx, config = {}) {
  ...
  const definitions = [
    ...
    defineTool({
      ...
      output: jsonOutput('Desktop visual Action Map created'),
      execute: async (args, exec) => await driver.visualActionMap(compact(args), exec),
    }),
    defineTool({
      name: 'desktop_click_visual_candidate',
      description: 'Click one D# from desktop_visual_action_map. The model supplies ONLY frameId/actionMapId/candidateId; Patrol clicks the program-computed candidate bbox center. Optional templatePath is for Patrol teaching: the exact candidate patch is saved before input so later runs can use desktop_click_visual_template.',
      parameters: {
        frameId: reqStr,
        actionMapId: reqStr,
        candidateId: reqStr,
      ...
      output: jsonOutput('Desktop visual candidate clicked'),
    ...
    defineTool({
      ...
      parameters: {
        templatePath: reqStr,
        expectedXRatio: reqNum,
        expectedYRatio: reqNum,
        searchWidthRatio: num,
        searchHeightRatio: num,
        minScore: num,
        button: { type: 'string', enum: ['left', 'right'] },
        processName: str,
        title: str,
      ...
      output: jsonOutput('Learned desktop visual template clicked'),
      execute: async (args, exec) => await driver.clickVisualTemplate(compact(args), exec),
    }),
    defineTool({
