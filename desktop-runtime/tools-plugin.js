...
export function apply(ctx, config = {}) {
  ...
  const definitions = [
    ...
    defineTool({
      ...
      output: jsonOutput('Desktop visual region focused'),
      execute: async (args, exec) => await driver.focusVisualRegion(compact(args), exec),
    }),
    defineTool({
      name: 'desktop_click_visual_point',
      description: 'DIRECTLY click a point bound to the latest CURRENT desktop_screenshot. No preview is required. For ordinary targets pass xRatio/yRatio of the full window, or imageX/imageY/imageWidth/imageHeight from the bounded full-window model image. For a tiny unlabeled icon first use desktop_focus_visual_region, then pass regionId plus imageX/imageY/imageWidth/imageHeight from that focused crop; Patrol reverses the crop + resize into the original full-window physical coordinates. The top-right window-control zone is rejected by default.',
      parameters: {
        xRatio: num,
        yRatio: num,
        imageX: num,
