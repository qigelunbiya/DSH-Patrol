      ...
      render: (_args, value) => [{
        ...
        text: [
          ...
          value.text && Number(value.confidence) >= 0.90
            ...
            : 'Both Windows OCR paths did not produce a strong CURRENT candidate. Refresh once or fall back to browser_capture_image_code_visual; do not submit a guess.',
        ].join('\n'),
      }],
    },
    presentCall: args => ({
...
async function recognizeFromCurrentPage(bridge, tabId, captured, args, exec, timeoutMs) {
  ...
  try {
    ...
    if (page && typeof page === 'object' && page.ok !== false && typeof page.text === 'string') knownText += `\n${page.text}`
  } catch {}
