...
const RECORDS_CLIENT_URL = new URL('./dashboard-records-client.js', import.meta.url)
      ...
      register(route) {
        ...
        return ctx.webServer.register({
          ...
          handler: async (req, res) => {
            ...
            if (url.searchParams.get('asset') === 'records') {
              const source = await readFile(RECORDS_CLIENT_URL, 'utf8')
              res.writeHead(200, {
                'content-type': 'text/javascript; charset=utf-8',
                'cache-control': 'no-store',
                'x-content-type-options': 'nosniff',
            ...
            const html = dashboardShell(prefix, mode)
            res.writeHead(200, {
...
function dashboardShell(prefix, mode = 'flows') {
  const clientSrc = mode === 'records' ? `${prefix}/ui?asset=records` : `${prefix}/ui?asset=client`
...
@media(max-width:900px){.app{padding:17px}.stats{grid-template-columns:repeat(2,1fr)}.meta-grid,.detail-grid{grid-template-columns:1fr}.toolbar{grid-template-columns:1fr 1fr!important}.toolbar input{grid-column:1/-1}.top{flex-direction:column}}
</style>
</head>
