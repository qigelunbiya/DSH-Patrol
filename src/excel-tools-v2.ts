...
- The registered patrol_excel_list / patrol_excel_inspect / patrol_excel_write tools use the resilient v2 bridge. Critical Excel COM operations use reflection late-binding instead of PowerShell's dynamic COM binder, avoiding common 参数类型不匹配 failures around Workbooks.Open, Worksheets.Item, Range, Value2, Formula and Save.
- Worksheet names are resolved by enumerating numeric worksheet indexes and comparing Name values; never pass a worksheet name as a COM Item index.
...
- Do not abandon an explicitly requested weekly-report write on an intermediate COM type mismatch. Only report Excel failure after the v2 tool itself returns a final staged error.`
...
export function registerPatrolExcelToolsV2(ctx: Context): () => void {
  ...
  const inspect = defineTool({
    ...
    description: 'Inspect an existing .xlsx in the CURRENT workspace with the resilient reflection-bound Excel v2 bridge. Prefer workbookRef from patrol_excel_list.',
    parameters: {
  ...
  const write = defineTool({
    ...
    description: 'Write selected cells to an inspected workspace .xlsx with reflection-bound COM calls. Requires explicit userRequestedWrite=true and a successful patrol_excel_inspect for the same workbook/worksheet in this runtime.',
    parameters: {
      workbookRef: { type: 'string' },
      filePath: { type: 'string' },
...
function Invoke-ComGet($target, [string]$name, [object[]]$arguments = @()) {
  return $target.GetType().InvokeMember($name, [Reflection.BindingFlags]::GetProperty, $null, $target, $arguments)
}
function Invoke-ComCall($target, [string]$name, [object[]]$arguments = @()) {
  return $target.GetType().InvokeMember($name, [Reflection.BindingFlags]::InvokeMethod, $null, $target, $arguments)
}
...
function Get-ComCount($target) { return [int](Invoke-ComGet $target 'Count') }
...
function Add-Warning($warnings, [string]$message) { if ($null -ne $warnings) { [void]$warnings.Add($message) } }
...
function Get-WorksheetByIndex($worksheets, [int]$index) { return Invoke-ComGet $worksheets 'Item' @($index) }
function Get-WorksheetByName($worksheets, [string]$name) {
  ...
  for ($index = 1; $index -le $count; $index++) {
    ...
    $matched = $false
    try { $matched = ([Convert]::ToString((Invoke-ComGet $candidate 'Name')) -eq $name) } catch {}
    if ($matched) { return $candidate }
    Release-ComObject $candidate
  }
  throw "Worksheet not found: $name"
}
function Get-WorksheetCell($sheet, [string]$address) { return Invoke-ComGet $sheet 'Range' @($address) }
function Set-CellValue2($cell, $value) { [void](Invoke-ComSet $cell 'Value2' $value) }
function Set-CellFormula($cell, [string]$value) { [void](Invoke-ComSet $cell 'Formula' $value) }
function Clear-Cell($cell) { [void](Invoke-ComCall $cell 'ClearContents') }
function Save-Workbook($workbook) { [void](Invoke-ComCall $workbook 'Save') }

function Get-CellText($cell, $warnings, [string]$address) {
  try { return [Convert]::ToString((Invoke-ComGet $cell 'Text')) } catch {
...
function Get-CellFormula($cell, $warnings, [string]$address) {
  try {
    if (-not [Convert]::ToBoolean((Invoke-ComGet $cell 'HasFormula'))) { return $null }
    return [Convert]::ToString((Invoke-ComGet $cell 'Formula'))
  } catch { Add-Warning $warnings "$address Formula: $(Error-Text $_)"; return $null }
}
function Get-CellMerge($cell, $warnings, [string]$address) {
  try {
    if (-not [Convert]::ToBoolean((Invoke-ComGet $cell 'MergeCells'))) { return $null }
    $area = Invoke-ComGet $cell 'MergeArea'
    try {
      $row = [int](Invoke-ComGet $area 'Row')
      $column = [int](Invoke-ComGet $area 'Column')
      $rows = Invoke-ComGet $area 'Rows'
      $columns = Invoke-ComGet $area 'Columns'
      try { return Get-A1Address $row $column ($row + (Get-ComCount $rows) - 1) ($column + (Get-ComCount $columns) - 1) }
...
function Get-CellNumberFormat($cell) { try { return [Convert]::ToString((Invoke-ComGet $cell 'NumberFormat')) } catch { return $null } }
function Get-CellBold($cell) {
  $font = $null
  try { $font = Invoke-ComGet $cell 'Font'; return [Convert]::ToBoolean((Invoke-ComGet $font 'Bold')) }
  catch { return $false }
  finally { Release-ComObject $font }
}
function Get-CellWrap($cell) { try { return [Convert]::ToBoolean((Invoke-ComGet $cell 'WrapText')) } catch { return $false } }

function Inspect-Sheet($sheet, [int]$maxRows, [int]$maxColumns) {
  $warnings = New-Object 'System.Collections.Generic.List[string]'
  $used = Invoke-ComGet $sheet 'UsedRange'
  try {
    $startRow = [int](Invoke-ComGet $used 'Row')
    $startColumn = [int](Invoke-ComGet $used 'Column')
    $rows = Invoke-ComGet $used 'Rows'
    $columns = Invoke-ComGet $used 'Columns'
    try {
    ...
    return [PSCustomObject]@{
      name = [Convert]::ToString((Invoke-ComGet $sheet 'Name'))
...
try {
  ...
  $stage = 'configure Excel.Application via reflection'
  [void](Invoke-ComSet $excel 'Visible' $false)
  [void](Invoke-ComSet $excel 'DisplayAlerts' $false)
  try { [void](Invoke-ComSet $excel 'AskToUpdateLinks' $false) } catch {}
  try { [void](Invoke-ComSet $excel 'AutomationSecurity' 3) } catch {}

  $stage = 'get Excel.Workbooks collection'
  $workbooks = Invoke-ComGet $excel 'Workbooks'
  $stage = "open workbook '$([IO.Path]::GetFileName([string]$payload.filePath))' via reflection"
  $workbook = Invoke-ComCall $workbooks 'Open' @([string]$payload.filePath)
  $worksheets = Invoke-ComGet $workbook 'Worksheets'
  ...
  if ([string]$payload.operation -eq 'inspect') {
    ...
    for ($index = 1; $index -le $worksheetCount; $index++) {
      ...
      try { $sheetNames.Add([Convert]::ToString((Invoke-ComGet $sheet 'Name'))) } finally { Release-ComObject $sheet }
    }
    ...
    try {
      ...
      $stage = 'save workbook via reflection'
      Save-Workbook $workbook
      [PSCustomObject]@{ operation='write'; path=[string]$payload.filePath; sheetName=[Convert]::ToString((Invoke-ComGet $sheet 'Name')); written=@($written); warnings=@($warnings) } | ConvertTo-Json -Depth 6 -Compress
  ...
  if ($null -ne $workbook) { try { [void](Invoke-ComCall $workbook 'Close' @($false)) } catch {}; Release-ComObject $workbook }
  if ($null -ne $workbooks) { Release-ComObject $workbooks }
  if ($null -ne $excel) { try { [void](Invoke-ComCall $excel 'Quit') } catch {}; Release-ComObject $excel }
