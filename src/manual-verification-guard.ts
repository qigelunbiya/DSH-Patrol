...
- OTP/one-time codes, device approvals, passkeys, QR confirmations, third-party reCAPTCHA/hCaptcha/Turnstile/Arkose widgets and genuinely unsupported verification can still use checkpoints immediately.
...
export function createManualVerificationGuard() {
  ...
  return (execution: any): string | undefined => {
    ...
    return [
      ...
      'OTP/one-time codes, device approvals and known third-party CAPTCHA widgets are not affected by this rule.',
    ].join(' ')
  }
}
