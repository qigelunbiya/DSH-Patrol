...
- patrol_visual_click_target 只能使用 patrol_observe(includeImage=true) 真正附加到模型上下文的 CURRENT image：observe 必须明确显示 MODEL-VISIBLE image attached、visualClickReady=true，并返回 visualFrameId（渲染为 Visual click frame READY）；DOM/OCR-only observation 不产生可用视觉点击凭证，严禁凭文本猜坐标。底层继续核对 tab、URL、scroll、zoom、viewport。visualAuthority=true 只用于用户明确要求视觉专用时，此时截图坐标拥有物理点击权，DOM/Accessibility 不得在点击前全局搜索后改写坐标；默认不传/false 时保持常规 hybrid，可用高置信 DOM/Accessibility 做局部校准。视频/卡片/详情导航还必须提供 expectedVisualText=截图中实际可见的完整标题，并在跳转后复核，不能只凭“click 已发出”宣称成功。
