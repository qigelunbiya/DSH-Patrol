...
- patrol_visual_click_target 只能使用 patrol_observe(includeImage=true) 真正附加到模型上下文的 CURRENT image：observe 必须明确显示 MODEL-VISIBLE image attached、visualClickReady=true，并返回 visualFrameId（渲染为 Visual click frame READY）；DOM/OCR-only observation 不产生可用视觉点击凭证，严禁凭文本猜坐标。模型可见图自带 XY/1000 归一化坐标网格，必须直接从网格读目标中心：xRatio=X/1000、yRatio=Y/1000；禁止根据操作系统屏幕宽度、CSS viewport 宽度或聊天界面里图片预览尺寸重新估算像素坐标。对小于约 100x60 CSS px 的图标/按钮，或 mark/right-click 已证明自由点有明显偏差时，优先使用 Set-of-Mark：patrol_observe(includeImage=true, actionMap=true, targetHint=当前完整业务目标, focusXRatio/focusYRatio=整页粗定位, focusWidthRatio≈0.24~0.32, focusHeightRatio≈0.24~0.36)。actionMap=true 时 targetHint 必填；结构化表格目标会先过滤到指定逻辑行+指定动作，再给剩余可交互矩形画 A1/A2/...。模型只负责在这个已经缩小的候选集中视觉选择编号，随后 patrol_visual_click_target(candidateId=A#, visualAuthority=true) 让浏览器点击该矩形安全点。不得读取 DOM 文本后替模型挑编号；DOM/CDP 在这里仅承担几何枚举。只有 action map 未覆盖目标时才退回局部 XY/1000 自由坐标。AUTO/HYBRID 只决定“是否选择视觉工具”，一旦调用 patrol_visual_click_target，现场教学点击始终以模型选择的截图坐标为物理权威，DOM/Accessibility 不得在点击前吸附或改写坐标；visualAuthority 参数仅保留兼容。visualFrameId 不再按时间或点击次数失效，同一 frame 可重复使用；但底层每次仍核对 tab、URL、scroll、zoom、viewport，页面几何变化后必须重新观察。需要验证点位时可用 pointerAction=mark/hover/right-click：mark 只画临时红色十字，hover 发 mouseMoved 并画标记，right-click 在同一点发可信右键；这三种都是诊断动作，不写 Runbook。视频/卡片/详情导航还必须提供 expectedVisualText=截图中实际可见的完整标题，并在跳转后复核，不能只凭“click 已发出”宣称成功。
