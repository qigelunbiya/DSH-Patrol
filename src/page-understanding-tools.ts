...
- patrol_visual_click_target 只能使用 patrol_observe(includeImage=true) 真正附加到模型上下文的 CURRENT image：observe 必须明确显示 MODEL-VISIBLE image attached、visualClickReady=true，并返回 visualFrameId；DOM/OCR-only observation 不产生可用视觉点击凭证。Browser visual plane 与 Desktop visual plane 必须彻底隔离。PRIMARY browser visual route 是 raw raster pixels：patrol_observe(includeImage=true, actionMap=false, targetHint=完整业务目标) 返回 CURRENT raster 的 modelRasterWidth/modelRasterHeight，模型直接从这张附图取目标中心 imageX/imageY，patrol_visual_click_target(imageX,imageY,imageWidth,imageHeight,visualAuthority=true) 由 runtime 依据同一 frame 保存的 raster + capture geometry 唯一映射到 viewport 并用 Chrome debugger trusted mouse 输入执行。禁止把 chat preview、OS 分辨率、devicePixelRatio、CSS viewport 或旧截图参与二次换算；小目标先 focused crop，再在局部 raster 上选 imageX/imageY。Action Map 改为显式 actionMap=true 的兼容路径，不再由 targetHint 自动开启；只有明确需要 A# 时才使用 candidateId。xRatio/yRatio 仅保留兼容，不作为新视觉教学首选。patrol_visual_click_target 正常省略 frameId，由运行时自动绑定当前 inspection 最近一次 model-visible browser frame。页面、滚动、缩放、导航或 viewport 变化后必须重新观察。视频/卡片/搜索结果/百科等 raw-pixel 导航仍必须提供截图中精确可见的 expectedVisualText 做目的地核验。
