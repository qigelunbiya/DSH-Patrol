...
- patrol_visual_click_target 必须使用 patrol_observe(includeImage=true) 返回的 CURRENT visualFrameId；底层会核对 tab、URL、scroll、zoom、viewport。NORMAL MODE 可以使用 DOM/Accessibility 做高置信校准；TEST MODE 使用 UI-TARS 风格 visual-authoritative grounding：截图坐标就是物理点击坐标，禁止在点击前用全局 DOM 搜索把它改写到另一个控件。targetHint 在 TEST MODE 主要用于业务结果校验和命中后 DOM/AX binding，不是坐标吸附器。
...
- 教学成功后的 browser_visual_click 会把视觉实际命中的 DOM/AX 目标反向绑定成 stable selector/fingerprint；重放优先执行这个 DOM selector，selector 漂移或目标只能通过视觉寻址时才恢复记录的 URL/scroll/viewport 并使用归一化 xRatio/yRatio。所有方法都必须以 CURRENT 业务状态验证为准，不能仅因为工具发出了 click 就宣称成功。
