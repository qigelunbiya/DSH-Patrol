...
- patrol_visual_click_target 必须使用 patrol_observe(includeImage=true) 返回的 CURRENT visualFrameId；底层会核对 tab、URL、scroll、zoom、viewport。TEST MODE 是 UI-TARS 风格 visual-grounding：截图坐标拥有物理点击权，DOM/Accessibility 不得在点击前全局搜索后改写坐标，targetHint 用于业务验证与点击后的 DOM/AX binding。NORMAL MODE 保持 hybrid，可用高置信 DOM/Accessibility 校准。
