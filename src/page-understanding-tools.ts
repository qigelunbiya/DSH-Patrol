...
- 对“某一行身份 + 行内动作”的表格目标（例如“10.192.3.174 这一行的 RDP/SSH”），禁止在整页 XY/1000 上凭肉眼估自由坐标，也禁止从全页几十个 A# 中猜一个编号。用户未指定方法时必须直接用 patrol_click_target，stepName 同时保留行身份和动作（例如“点击 10.192.3.174 行的 RDP”）、locatorText 只写动作“RDP”；底层 row-context resolver 每次按 CURRENT 逻辑行重新解析，并把这类步骤保存为 semantic replay，不持久化脆弱 nth-of-type 行号。只有用户明确要求“只用视觉”时，才允许 patrol_visual_click_target(..., visualAuthority=true)：先 patrol_observe(includeImage=true, actionMap=true, targetHint="10.192.3.174 行的 RDP", focus...)；action map 会在编号前用 CURRENT 行身份+动作过滤掉其他行，并且点击前还会再次校验 candidate 的 logical-row context，错误 A# 会在物理点击前被拒绝。right-click/mark 诊断也遵守同一规则。
...
- patrol_visual_click_target 只能使用 patrol_observe(includeImage=true) 真正附加到模型上下文的 CURRENT image：observe 必须明确显示 MODEL-VISIBLE image attached、visualClickReady=true，并返回 visualFrameId（渲染为 Visual click frame READY）；DOM/OCR-only observation 不产生可用视觉点击凭证，严禁凭文本猜坐标。模型可见图自带 XY/1000 归一化坐标网格，必须直接从网格读目标中心：xRatio=X/1000、yRatio=Y/1000；禁止根据操作系统屏幕宽度、CSS viewport 宽度或聊天界面里图片预览尺寸重新估算像素坐标。actionMap=true 时 targetHint 必填；除结构化表格的“行身份+动作”过滤外，CURRENT Action Map 还会用完整 targetHint 的可见文字/局部上下文把“发售版本”“我的任务”等目标缩到小候选集，无法形成小而明确的集合时才保留全量候选；候选数 <=16 时额外附加 ACTION MAP TARGET ZOOM 第二张图，把每个 A# 周围的真实页面像素放大并标出精确 safe point，模型只用这张放大图选 A#，绝不把放大图像素当点击坐标。TEST MODE 中任何浏览器视觉左键都不允许直接落地，不只限于小目标：先用 patrol_visual_click_target(pointerAction=mark, candidateId=A# 或 xRatio/yRatio, targetHint=...) 在 CURRENT 页面画红色十字；工具会返回 browser-preview-*。小于约 120x72 CSS px 的按钮/图标、相邻标签的 x、覆盖/重叠输入框、密集目录项尤其必须如此。随后必须再 patrol_observe(includeImage=true) 用视觉确认十字确实落在目标内部；确认无误后调用 patrol_visual_click_target(previewId=browser-preview-*, targetHint=同一业务目标, ...)。previewId 把真实点击绑定到刚刚标记的同一个 frame 坐标，禁止重新计算/重传坐标造成二次漂移。若十字不准，换候选或自由 XY 重新 mark，绝不能先点再试错。Action Map 与自由 XY 是并列视觉方案：Set-of-Mark 适合离散可交互控件，自由 XY 适合 Action Map 未覆盖的自绘/特殊区域；任何一种不准都可切换另一种，但切换后仍先 mark。AUTO/HYBRID 只决定“是否选择视觉工具”，一旦调用 patrol_visual_click_target，现场教学点击始终以模型选择且已预览的截图坐标为物理权威，DOM/Accessibility 不得在点击前吸附或改写坐标；visualAuthority 参数仅保留兼容。visualFrameId 不再按时间或点击次数失效，同一 frame 可重复使用；但底层每次仍核对 tab、URL、scroll、zoom、viewport，页面几何变化后必须重新观察。hover/right-click 仍是诊断动作，不写 Runbook。视频/卡片/详情导航还必须提供 expectedVisualText=截图中实际可见的完整标题，并在跳转后复核，不能只凭“click 已发出”宣称成功。
...
function structuredRowFreePointIssue(args: Record<string, unknown>): string | undefined {
  ...
  const candidateId = cleanString(args.candidateId)
  const explicitVisualOnly = args.visualAuthority === true
  if (candidateId && explicitVisualOnly) return undefined
  return [
    'DSH Patrol structured-row precision guard：本次视觉点击未执行。',
    `目标同时包含行身份 ${identity} 和行内动作 ${action}。这种目标不能从整页自由坐标或未限定的 A# 中猜，因为相邻数据行可能具有相同的 ${action} 文本。`,
    `用户未指定操作方法时，必须改用 patrol_click_target：stepName 保留“${identity} 行的 ${action}”，locatorText=${JSON.stringify(action)}；CURRENT row-context resolver 会绑定正确逻辑行，并保存 semantic replay，而不是 nth-of-type 行号。`,
    `只有用户明确要求只用视觉时才允许候选点击：先 patrol_observe(includeImage=true, actionMap=true, targetHint=${JSON.stringify(`${identity} 行的 ${action}`)}, focus...)，再把过滤后的 A# 传给 patrol_visual_click_target，并显式 visualAuthority=true。错误行 candidate 会在物理输入前被拒绝。`,
  ].join(' ')
}
