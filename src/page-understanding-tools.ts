...
- 对“某一行身份 + 行内动作”的表格目标（例如“10.192.3.174 这一行的 RDP/SSH”），禁止在整页 XY/1000 上凭肉眼估一个自由坐标：这种目标的行高很小，模型可能认对 RDP 却把 Y 坐标落到上方筛选输入框。用户未指定方法时直接用 patrol_click_target，stepName 同时保留行身份和动作（例如“点击 10.192.3.174 行的 RDP”）、locatorText 只写动作“RDP”，底层 row-context resolver 会绑定正确逻辑行；若用户明确只允许视觉，则必须先 focused actionMap，再用 candidateId=A#，不得使用自由 xRatio/yRatio。right-click/mark 诊断同样必须使用 actionMap candidate，不能用粗略网格坐标。
...
function structuredRowFreePointIssue(args: Record<string, unknown>): string | undefined {
  if (cleanString(args.candidateId)) return undefined
  ...
  return [
    'DSH Patrol structured-row precision guard：本次自由坐标视觉点击未执行。',
    `目标同时包含行身份 ${identity} 和行内动作 ${action}；整页 XY 自由点容易落到相邻行或表头筛选框。`,
    `默认/AUTO 模式请改用 patrol_click_target：stepName 保留“${identity} 行的 ${action}”，locatorText=${JSON.stringify(action)}，让 CURRENT row-context resolver 绑定正确行。`,
    '如果用户明确要求只用视觉，则先 patrol_observe(includeImage=true, actionMap=true, focus...) 获取局部 A# 编号，再用 patrol_visual_click_target(candidateId=A#)；mark/right-click 诊断也必须对 A# 执行。',
  ].join(' ')
}
