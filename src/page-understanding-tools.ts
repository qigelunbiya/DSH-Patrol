...
- DOM/semantic、CURRENT selector、浏览器视觉都是可选执行方法，不规定固定优先级，也不要求必须先 DOM 再视觉。模型根据当前场景自行选择最可靠的方法：文本/结构清晰时可直接 patrol_click_target；视觉布局更直观、DOM 难以表达或模型判断视觉更合适时，可以直接 patrol_observe(includeImage=true) → patrol_visual_click_target。patrol_analyze_step 是可选结构化证据工具，不是视觉调用的前置许可。
...
- patrol_visual_click_target 只能使用 patrol_observe(includeImage=true) 真正附加到模型上下文的 CURRENT image：observe 必须明确显示 MODEL-VISIBLE image attached、visualClickReady=true/Visual click frame READY；DOM/OCR-only observation 不产生可用视觉点击凭证，严禁凭文本猜坐标。底层继续核对 tab、URL、scroll、zoom、viewport。TEST MODE 是 UI-TARS 风格 visual-grounding：截图坐标拥有物理点击权，DOM/Accessibility 不得在点击前全局搜索后改写坐标。视频/卡片/详情导航还必须提供 expectedVisualText=截图中实际可见的完整标题，执行器只验证该坐标的局部上下文与标题一致并在跳转后复核，不做坐标吸附。评论输入框可在视觉点击之后对同一局部组件新挂载的 Shadow DOM editable 做焦点恢复。NORMAL MODE 保持 hybrid，可用高置信 DOM/Accessibility 校准。
...
- 教学成功后的 browser_visual_click 会反向学习 visual hit 对应的 semantic locator / stable selector；重放顺序是 learned semantic → learned selector → guarded visual geometry。也就是说 TEST MODE 教学可以纯视觉完成复杂 UI，未来复用时仍尽量转换为 DOM 命令。所有方法都必须以 CURRENT 业务状态验证为准，不能仅因为工具发出了 click 就宣称成功。
