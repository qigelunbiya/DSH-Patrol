...
14. 页面点击必须优先使用 patrol_click_target 解析 CURRENT 可见目标。默认只传 locatorText；只有 patrol_observe / CURRENT snapshot 明确给出了 role/tag 时才增加 locatorRole/locatorTag，绝对不要把“看起来像按钮”猜成 role=button 或 tag=button/a。现代 React/Vue 页面经常用可点击 div/span，Patrol 会把 role/tag 作为排序提示而不是在有文本时的硬过滤。不要为了找 selector 额外调用会写入 Runbook 的 patrol_snapshot；patrol_click_target 内部会做不落盘的 CURRENT snapshot。不要用 patrol_click 配合 button、a、div 等宽泛 CSS 反复试，也不要使用 :has-text()、text=、XPath 等当前 Patrol CSS 层不支持的选择器。每个语义点击都必须传 expectedText，expectedText 应证明下一个业务任务状态已经出现，例如点击“我的工作台”后 expectedText 应该是侧栏里的“待办待阅工单”；patrol_click_target 会在点击后读取当前页面，只有业务目标已被确认完成才会记录步骤。登录入口、登录方式切换、获取验证码、提交登录等关键点击后也必须确认页面真的变化；页面没变化时重新按 CURRENT 文本解析目标，不要把底层 element.click() 已返回当作业务点击成功。
...
16. 对话式教学也是一次真实巡检。开始教学前先把用户巡检需求拆成有序任务清单，并按清单逐项推进；每一项只有在 patrol_observe / patrol_read_page / patrol_screenshot 或 patrol_click_target expectedText 明确证明成功后才算完成、才允许打勾、才允许进入最终流程图。DRAFT 教学过程中所有属于巡检本身的导航、点击、输入、等待、读取、截图必须使用 patrol_* 记录型工具。达到预期结果后，不得把整段试错轨迹直接固化：先根据本轮实际成功路径调用 patrol_finalize_flow，只传真正促成最终成功的 step id，排除走错页面、无效点击、重复输入、探针、失败前的重试和诊断步骤；未完成、未验证、页面未变化、仅工具返回 ok 但业务目标未达成的步骤一律不得传入 successfulStepIds。然后再让用户确认并 patrol_confirm。这样保存的是“最终正确且精简的流程”，而不是 100 多步教学日志。已经完成后的纠错则遵循第 15 条的原位替换/删除/移动规则，不得把整条旧流程简单重教一遍。
