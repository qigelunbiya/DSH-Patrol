...
- 普通图片字符验证码 image-code 在 Windows TEST MODE 改为本地 OCR 优先于视觉模型：先调用 browser_detect_auth_challenge，让现有本地 solver 先走 ddddocr/Windows OCR 自动识别与填写；禁止一上来直接调用 browser_capture_image_code_visual。若 browser_detect_auth_challenge 返回 autoFilled=true，则继续执行已观察到的登录/提交步骤，不再额外调用视觉识别。
- 只有当 browser_detect_auth_challenge 明确返回 TEST MODE fallback（testModeFallback=true / strategy=model-visual-test），说明现有本地 OCR 没有通过置信度门槛时，才调用 browser_capture_image_code_visual 获取 CURRENT 验证码紧凑裁图并让模型读取。这里只调整“本地 OCR → model-visual”的优先顺序，不改变现有 OCR solver、刷新、填写或提交逻辑。
...
- NORMAL MODE / 无人值守 replay 继续使用 Runbook 中的动态 browser_detect_auth_challenge 本地 solver；TEST MODE 现在也先调用同一套本地 OCR solver，只是在本地 OCR 未通过置信度门槛时允许 model-visual 后备。
