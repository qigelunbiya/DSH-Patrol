...
- patrol_visual_click_target 只有在 patrol_observe(includeImage=true) 明确返回 MODEL-VISIBLE image attached 且 visualClickReady=true/Visual click frame READY 后才允许使用；若只返回 DOM/OCR、imageStatus 不是 attached 或出现 VISUAL CLICK DISABLED，禁止猜坐标，也禁止声称自己正在用视觉。浏览器视觉点击的 PRIMARY 路线改为 raw model-raster pixel：先 patrol_observe(includeImage=true, actionMap=false, targetHint=当前业务目标)，观察结果会明确 modelRasterWidth/modelRasterHeight；模型直接在这张 CURRENT 附图上给出目标中心 imageX/imageY，并把 imageWidth/imageHeight 原样带给 patrol_visual_click_target。运行时用绑定 frame 保存的 raster/capture geometry 做唯一一次 image pixel -> viewport 映射，然后必须经 Chrome debugger 发 trusted mouse click；禁止模型手工把 imageX/imageY 再换算成 CSS 像素、DPR 或浏览器 viewport 坐标。目标很小时优先对同一 CURRENT 页面做 focused crop，再在局部 raster 上给 imageX/imageY。XY/1000 网格仅作为视觉辅助，xRatio/yRatio 只保留旧兼容 fallback。Action Map 不再因 targetHint 自动开启；只有显式 actionMap=true 才进入 A# compatibility path。对于“IP/主机名 + RDP/SSH/VNC”等表格行目标，action map 会先按 CURRENT logical-row identity + action 过滤掉其它行，再给剩余真实可交互矩形画 A1/A2/... 编号框；模型只在这个目标候选集中看图选编号；当候选不超过 16 个时，observe 还会附加第二张 ACTION MAP TARGET ZOOM 放大图，每格显示一个候选的真实局部像素、A# 和精确 safe-point 十字，只用来决定 WHICH A#，绝不能从放大图换算点击 XY。显式 actionMap=true 时仍可选 A# 后 patrol_visual_click_target(candidateId=A#,visualAuthority=true)，但这只是兼容路径；普通浏览器视觉点击优先 imageX/imageY。通常不要再传 frameId：运行时自动绑定该 inspection 最近一次真正附加到模型的 CURRENT browser visual frame，避免 frameId/截图文件名抄错。若 observe 同时返回 CURRENT A# bindings 文本摘要，必须用摘要与图上的编号交叉核对；明确文字/章节/搜索结果/标签关闭目标出现 STRICT TARGET MISS 时，禁止改猜邻近 A# 或自由 XY，刷新/重聚焦后重新观察。只有模型自己对候选是否正确仍不确定时，才可选用 pointerAction=mark 做诊断。点击前底层还会核验该 candidate 的 rowContext；若候选来自错误主机/错误动作，会在任何物理输入之前拒绝。对于普通非结构化控件，DOM/CDP 仍只提供候选矩形和 verified safe-point，不替模型决定业务目标；模型只选 A#。只有 action map 没有覆盖目标（例如 canvas/特殊自绘区域）时，才退回局部 XY/1000 自由点。若 mark/right-click 已证明自由点偏左、偏上或落到相邻控件，优先重新生成 actionMap，而不是继续加减几十像素。已获得的 visualFrameId 没有 120 秒 TTL、没有单次消费、没有固定帧数/点击次数上限；同一个 frame 在 CURRENT tab/URL/scroll/zoom/viewport 未变化时可以反复视觉点击。TEST MODE 不再强制视觉左键先 mark/preview。CURRENT 截图里目标位置或 Action Map candidateId 已经明确时应直接执行左键，避免“定位正确→重新截图→执行时偏离”的额外链路。pointerAction=mark/hover/right-click 只保留为模型不确定时的可选诊断动作，不记录 Runbook。CDP hover/right-click 作用于浏览器输入系统，不保证 Windows 的硬件鼠标光标图标会跟着移动，因此肉眼校准优先看红色标记/右键菜单，而不是 OS 光标。页面滚动、缩放、导航、切页或 viewport 变化后，旧坐标会因 CURRENT 几何不一致被拒绝，此时重新观察。对于视频/卡片/详情导航，candidateId 点击自动携带候选自身的 CURRENT visible text/title/aria 做目标与目的地核验；只有 free-XY 导航才必须填写模型从截图实际看见的完整 expectedVisualText。若点击打开新标签页，Patrol 必须跟随新标签页做目标标题验证，不能继续读取旧首页。评论/回复输入框允许在可靠视觉点击之后，仅在点击点附近对新挂载的 Shadow DOM editable 做一次局部焦点恢复。视觉截图也没有固定次数上限；每次新图前 Patrol 会把历史工具图片从模型可见上下文中 offload。
