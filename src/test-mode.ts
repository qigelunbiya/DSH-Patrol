...
- patrol_visual_click_target 只有在 patrol_observe(includeImage=true) 明确返回 MODEL-VISIBLE image attached 且 visualClickReady=true/Visual click frame READY 后才允许使用。TEST MODE 现场视觉左键现在只有两条主路径：① 大搜索框/搜索栏/输入框/明显大按钮等宽容目标，必须直接使用 CURRENT 附图的 imageX/imageY；TEST MODE 不再允许模型把截图像素手工换算成 xRatio/yRatio。② 搜索结果链接、百科标题、章节/目录/菜单/标签项、x/×/关闭/移除、小图标等精细目标，必须先用整页 CURRENT 图确定粗中心，再做 focused patrol_observe(includeImage=true, focusXRatio=..., focusYRatio=..., focusWidthRatio≈0.34~0.48, focusHeightRatio≈0.30~0.42, pixelActionMap=true, actionMap=false, targetHint=同一业务目标)，然后只用 pixelCandidateId=B# 点击。注意“百度搜索结果页搜索框”实际目标是搜索框，不得因为上下文含“搜索结果”而误判成搜索结果链接；相反“龙之信条2 - 百度百科 搜索结果链接”才属于 B# 精细目标。B# bbox/center 由程序从 CURRENT model raster 计算并唯一映射到 capture/viewport。页面滚动、缩放、导航、切页或 viewport 变化后必须重新观察。导航/搜索结果/百科等点击仍必须提供 CURRENT 截图中精确可见的 expectedVisualText 做事后目的地核验。这些规则是运行时硬门禁：精细目标没有 B# 会在任何真实鼠标输入前拒绝；TEST MODE live xRatio/yRatio 对所有现场左键都直接拒绝。
