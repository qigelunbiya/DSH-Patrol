...
- patrol_visual_click_target 只有在 patrol_observe(includeImage=true) 明确返回 MODEL-VISIBLE image attached 且 visualClickReady=true/Visual click frame READY 后才允许使用；若只返回 DOM/OCR、imageStatus 不是 attached 或出现 VISUAL CLICK DISABLED，禁止猜 xRatio/yRatio，也禁止声称自己正在用视觉。CURRENT 模型截图带 XY/1000 坐标网格；视觉选点必须优先读取网格位置，再按 xRatio=X/1000、yRatio=Y/1000 调用，不要根据“屏幕大约 1425px/2048px”之类主观估算重新换算。对点赞图标、发布按钮、三点菜单等小目标，禁止直接拿整页 1024px 截图反复微调坐标：先用整页图得到粗略中心，再调用 patrol_observe(includeImage=true, focusXRatio=粗略X, focusYRatio=粗略Y, focusWidthRatio≈0.24~0.32, focusHeightRatio≈0.24~0.36) 获取局部放大 CURRENT frame；最终 patrol_visual_click_target 的 xRatio/yRatio 必须从这个局部图自己的 XY/1000 网格重新读取。第一次 mark/right-click 发现偏左、偏上或落到相邻控件时，也必须切换局部放大 frame，不要继续在整页 frame 上凭感觉加减几十像素。已获得的 visualFrameId 没有 120 秒 TTL、没有单次消费、没有固定帧数/点击次数上限；同一个 frame 在 CURRENT tab/URL/scroll/zoom/viewport 未变化时可以反复视觉点击。若需要验证定位而不触发正常左键业务动作，可用 pointerAction=mark 在精确点位画临时红色十字，pointerAction=hover 发可信 mouseMoved 并画标记，pointerAction=right-click 发可信右键并画标记；这些只用于调试定位，不记录 Runbook。CDP hover/right-click 作用于浏览器输入系统，不保证 Windows 的硬件鼠标光标图标会跟着移动，因此肉眼校准优先看红色标记/右键菜单，而不是 OS 光标。页面滚动、缩放、导航、切页或 viewport 变化后，旧坐标会因 CURRENT 几何不一致被拒绝，此时重新观察。对于视频/卡片/详情导航，expectedVisualText 必须填写模型从截图实际看见的完整标题；若点击打开新标签页，Patrol 必须跟随新标签页做目标标题验证，不能继续读取旧首页。评论/回复输入框允许在可靠视觉点击之后，仅在点击点附近对新挂载的 Shadow DOM editable 做一次局部焦点恢复。视觉截图也没有固定次数上限；每次新图前 Patrol 会把历史工具图片从模型可见上下文中 offload。
