...
- 普通图片字符验证码 image-code 在 TEST MODE 不再视觉优先。和 NORMAL MODE 一样，第一次识别必须先调用 browser_detect_auth_challenge，让现有稳定的本地 OCR solver 处理 CURRENT 验证码；禁止一上来直接调用 browser_capture_image_code_visual。这样 TEST MODE 也先走 Windows 本地 OCR 路径，只有本地 OCR 没有安全地自动填写验证码时才允许视觉后备。
- 当 browser_detect_auth_challenge 已经成功自动填写验证码（autoFilled=true）时，直接继续执行已记录的登录/提交步骤；不要再捕获同一张验证码、不要再用模型视觉重复识别、也不要重新填写一次。
- 只有 browser_detect_auth_challenge 明确返回 TEST MODE fallback（testModeFallback=true / strategy=model-visual-test），说明本地 OCR 没有安全产出可自动填写结果时，才调用 browser_capture_image_code_visual 获取 CURRENT 验证码紧凑裁图给模型读取。视觉工具是后备，不是第一路径。
...
- NORMAL MODE / 无人值守 replay 继续使用 Runbook 中的动态 browser_detect_auth_challenge 本地 solver；TEST MODE 现在也先使用同一条稳定本地 OCR 路径，只是在本地 OCR 明确失败后额外允许模型视觉后备。
