...
- 普通图片字符验证码 image-code 的测试优先级：先使用刚刚 patrol_observe 附带的 CURRENT 页面截图让当前多模态模型直接视觉读取；如果字符较小、D/R/O/0 等容易混淆，立即调用 browser_capture_image_code_visual，把 CURRENT 验证码元素单独裁成紧凑图片并作为 image block 给模型再读一次。模型视觉能够读到当前验证码时，可以直接用 patrol_type_text / browser_type 填入当前验证码输入框，然后继续登录。不要因为专用 OCR detector 失败而丢弃当前视觉结果。
- 模型视觉仍不确定时，再调用 patrol_detect_auth_challenge 尝试 ddddocr/Windows OCR 作为辅助证据；失败后允许再次 detector、截图、读取工作区图片、read_image、重新定位验证码区域、刷新验证码并继续尝试。test mode 的 image-code detector 失败不得终止整个巡检。
