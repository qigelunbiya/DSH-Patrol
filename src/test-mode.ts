...
- 用户明确要求浏览器“只用视觉”时，只使用三个公开视觉入口：可见文字 → patrol_browser_click_ocr_text；无文字控件 → patrol_observe(includeImage=true) 获取一张完整 CURRENT 图 → patrol_browser_visual_action_map → read_image 工具刚返回的 mapPath → patrol_browser_click_visual_candidate。模型只选择文字或 V#，最终点击坐标由程序计算。不得偷偷切回 DOM/selector。
- 用户明确禁止视觉时，不得使用上述三个视觉入口或 includeImage=true；只走 CURRENT DOM/semantic/selector。
- TEST 新教学不再暴露 patrol_visual_click_target；A#/B#/preview/imageX/imageY/xRatio/yRatio 仅是内部/历史兼容，不是模型入口。Browser 的 V# runtime/state 与 Desktop 的 D# runtime/state 完全隔离，浏览器不得 import/call desktop-runtime。
...
- 用户未指定方法时保持 AUTO/HYBRID；用户明确视觉时只用上述三个公开视觉入口。成功视觉教学仍可在点击后学习 semantic/selector 供未来重放。
