...
- 若用户明确要求“只用视觉模型/必须视觉巡检/不要 DOM 点击”等视觉专用语义，本轮业务动作必须由 patrol_observe(includeImage=true) 的 CURRENT model-visible screenshot 驱动，并在 patrol_visual_click_target 传 visualAuthority=true。patrol_visual_click_target 的现场点击本身始终采用 UI-TARS 风格 coordinate-authoritative：即使调用方漏传该兼容参数，也不得让 DOM/Accessibility 在点击前全局搜索、吸附或改写模型选中的坐标；DOM 只允许在点击后用于命中身份学习与结果验证。若 CURRENT 图片没有成功附加或 visualClickReady=false，必须重新获取可见截图或报告视觉路径阻塞，禁止偷偷改用 DOM 点击完成任务。
...
- patrol_visual_click_target 只有在 patrol_observe(includeImage=true) 明确返回 MODEL-VISIBLE image attached 且 visualClickReady=true/Visual click frame READY 后才允许使用；若只返回 DOM/OCR、imageStatus 不是 attached 或出现 VISUAL CLICK DISABLED，禁止猜 xRatio/yRatio，也禁止声称自己正在用视觉。已获得的 visualFrameId 没有 120 秒 TTL、没有单次消费、没有固定帧数/点击次数上限；同一个 frame 在 CURRENT tab/URL/scroll/zoom/viewport 未变化时可以反复视觉点击。页面滚动、缩放、导航、切页或 viewport 变化后，旧坐标会因 CURRENT 几何不一致被拒绝，此时重新观察。对于视频/卡片/详情导航，expectedVisualText 必须填写模型从截图实际看见的完整标题；若点击打开新标签页，Patrol 必须跟随新标签页做目标标题验证，不能继续读取旧首页。评论/回复输入框允许在可靠视觉点击之后，仅在点击点附近对新挂载的 Shadow DOM editable 做一次局部焦点恢复。视觉截图也没有固定次数上限；每次新图前 Patrol 会把历史工具图片从模型可见上下文中 offload。
