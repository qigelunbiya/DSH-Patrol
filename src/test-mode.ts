...
- patrol_visual_click_target 只有在 patrol_observe(includeImage=true) 明确返回 MODEL-VISIBLE image attached 且 visualClickReady=true/Visual click frame READY 后才允许使用；若只返回 DOM/OCR、imageStatus 不是 attached 或出现 VISUAL CLICK DISABLED，禁止猜 xRatio/yRatio，也禁止声称自己正在用视觉。CURRENT 模型截图带 XY/1000 坐标网格；视觉选点必须优先读取网格位置，再按 xRatio=X/1000、yRatio=Y/1000 调用，不要根据“屏幕大约 1425px/2048px”之类主观估算重新换算。对点赞图标、发布按钮、三点菜单等小目标，不再把“连续坐标回归”作为首选。先用整页图得到粗略中心，再调用 patrol_observe(includeImage=true, actionMap=true, targetHint=当前完整业务目标, focusXRatio=粗略X, focusYRatio=粗略Y, focusWidthRatio≈0.24~0.32, focusHeightRatio≈0.24~0.36)。actionMap=true 时 targetHint 必填。对于“IP/主机名 + RDP/SSH/VNC”等表格行目标，action map 会先按 CURRENT logical-row identity + action 过滤掉其它行，再给剩余真实可交互矩形画 A1/A2/... 编号框；模型只在这个目标候选集中看图选编号；当候选不超过 16 个时，observe 还会附加第二张 ACTION MAP TARGET ZOOM 放大图，每格显示一个候选的真实局部像素、A# 和精确 safe-point 十字，只用来决定 WHICH A#，绝不能从放大图换算点击 XY。选定后先 patrol_visual_click_target(pointerAction=mark,candidateId=A#,visualAuthority=true)，再 observe 验证红十字，最后用 previewId 左键，不再猜 xRatio/yRatio。点击前底层还会核验该 candidate 的 rowContext；若候选来自错误主机/错误动作，会在任何物理输入之前拒绝。对于普通非结构化小控件，DOM/CDP 仍只提供矩形几何，不替模型决定业务目标。只有 action map 没有覆盖目标时，才退回局部 XY/1000 自由点。若 mark/right-click 已证明自由点偏左、偏上或落到相邻控件，优先重新生成 actionMap，而不是继续加减几十像素。已获得的 visualFrameId 没有 120 秒 TTL、没有单次消费、没有固定帧数/点击次数上限；同一个 frame 在 CURRENT tab/URL/scroll/zoom/viewport 未变化时可以反复视觉点击。TEST MODE 中所有浏览器视觉左键都必须先校准，pointerAction=mark 不再只是可选调试：无论选点来自 Action Map candidateId 还是自由 XY，第一次都只能 mark，在 CURRENT 页面画红色十字并返回 browser-preview-*；随后立刻 patrol_observe(includeImage=true) 重新看同一页面，确认十字确实位于目标控件可点击内部，再以同一 targetHint 调用 patrol_visual_click_target(previewId=browser-preview-*) 执行真实左键。未持有 previewId 的视觉 left-click 会被工具拒绝。pointerAction=hover/right-click 仍是可选诊断动作，不记录 Runbook。CDP hover/right-click 作用于浏览器输入系统，不保证 Windows 的硬件鼠标光标图标会跟着移动，因此肉眼校准优先看红色标记/右键菜单，而不是 OS 光标。页面滚动、缩放、导航、切页或 viewport 变化后，旧坐标会因 CURRENT 几何不一致被拒绝，此时重新观察。对于视频/卡片/详情导航，expectedVisualText 必须填写模型从截图实际看见的完整标题；若点击打开新标签页，Patrol 必须跟随新标签页做目标标题验证，不能继续读取旧首页。评论/回复输入框允许在可靠视觉点击之后，仅在点击点附近对新挂载的 Shadow DOM editable 做一次局部焦点恢复。视觉截图也没有固定次数上限；每次新图前 Patrol 会把历史工具图片从模型可见上下文中 offload。
