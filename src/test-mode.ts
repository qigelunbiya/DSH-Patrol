...
- 在用户没有指定浏览器操作方式的默认 AUTO/HYBRID 策略下，TEST MODE 仍允许 semantic/selector 等 CURRENT 页面恢复通道；但一旦用户明确指定视觉专用，现场点击统一使用 patrol_browser_click_ocr_text 或 patrol_browser_visual_action_map → patrol_browser_click_visual_candidate。patrol_visual_click_target、browser_visual_click 仅作为底层兼容/重放引擎，不再作为模型新教学入口。成功视觉教学仍把实际命中后的 learned semantic → learned selector → guarded visual geometry 保存给未来无人值守重放；这不改变当前教学轮次的纯视觉要求。
