...
- patrol_visual_click_target 只有在 patrol_observe(includeImage=true) 明确返回 MODEL-VISIBLE image attached 且 visualClickReady=true/Visual click frame READY 后才允许使用。浏览器视觉点击现在优先采用两阶段 Browser Pixel Grounding，而不是整页自由坐标：① 先 patrol_observe(includeImage=true, actionMap=false, pixelActionMap=false, targetHint=当前业务目标) 看 CURRENT 整页，只确定粗区域；大搜索框/大按钮可直接 imageX/imageY。② 对文字链接、菜单项、小图标、标签 x/×、密集列表等容易点错的目标，必须根据整页图给出粗 focusXRatio/focusYRatio，再调用 patrol_observe(includeImage=true, focusXRatio=..., focusYRatio=..., focusWidthRatio≈0.18~0.32, focusHeightRatio≈0.18~0.30, pixelActionMap=true, targetHint=同一业务目标)。focused crop 会放大同一 CURRENT 页面，并用纯截图像素生成 B1/B2/... 候选；模型只选红框/绿色中心十字落在目标内部的 B#，随后 patrol_visual_click_target(pixelCandidateId=B#, visualAuthority=true)。B# 的 bbox/center 由程序从 model raster 像素计算并唯一映射到 capture/viewport，禁止模型再次估 imageX/xRatio/CSS/DPR/OS 坐标。focused crop 在未显式关闭 pixelActionMap 且未启用 DOM actionMap 时会自动生成 B#。只有 B# 没有覆盖目标且目标足够大时才回退 direct imageX/imageY。A# DOM Action Map 仍保留给显式 actionMap=true 的兼容/结构化表格场景，不再作为普通小目标主方案。frameId 正常省略，由当前 inspection 最近一次 model-visible frame 自动绑定。页面滚动、缩放、导航、切页或 viewport 变化后必须重新观察。导航/搜索结果/百科等 B# 或 raw-pixel 点击仍必须提供截图中精确可见的 expectedVisualText 做点击后目的地验证。 这条规则现在是运行时硬门禁而不是建议：TEST MODE 中，搜索结果/百科链接、章节/目录/菜单/标签项、x/×/关闭/移除、小图标等精细目标如果没有 pixelCandidateId=B#，patrol_visual_click_target 会在任何 browser_visual_click/真实鼠标输入之前直接拒绝；xRatio/yRatio、imageX/imageY、previewId 和旧 A# 都不能绕过。第一次尚未提供坐标时也只会返回 focused pixelActionMap → B# 这一条恢复路径。
