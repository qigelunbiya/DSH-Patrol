...
- CURRENT 页面需要点击按钮、链接、选项卡、弹窗操作时，优先使用 patrol_click_target，不要再用 patrol_click + 猜测的 button/a/div 等宽泛 CSS 反复试。patrol_click_target 可以只给 locatorText，也可以在 CURRENT 观察明确给出时加 locatorRole/locatorTag；selector 只是可选提示。它会先解析唯一可见目标再执行 browser_click；如果 atomic semantic click 瞬态不可用但已有 selector hint，它会在同一次受控调用内重新计数并仅对唯一 selector 安全降级，不要另起 patrol_click 重复业务动作。
