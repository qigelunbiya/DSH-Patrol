...
- 若用户明确要求“只用视觉模型/必须视觉巡检/不要 DOM 点击”等视觉专用语义，本轮浏览器业务点击只能走新的 Browser 独立视觉链：可见文字用 patrol_browser_click_ocr_text；无文字/小图标/空搜索框等用 patrol_observe(includeImage=true) 获取完整 CURRENT 截图 → patrol_browser_visual_action_map 在同一 frame 上生成 V1/V2/... → read_image 精确读取该 Action Map → patrol_browser_click_visual_candidate 点击 V#。不得因为视觉失败偷偷改用 patrol_click_target/patrol_click/selector。DOM/Accessibility 只允许在真实视觉点击之后用于命中身份学习、状态验证和未来重放；不能在 live 点击前替换模型从截图选中的目标。
- 若用户明确要求“禁止视觉/不要视觉模型/只用 DOM”等非视觉语义，本轮不得调用 patrol_observe(includeImage=true)、patrol_browser_visual_action_map、patrol_browser_click_visual_candidate、patrol_browser_click_ocr_text 或任何基于截图几何的页面动作；只能使用 CURRENT DOM/semantic/selector。不得因为 DOM 难定位就静默切换视觉。
- 浏览器 TEST 视觉点击正式推翻旧 OCR-first/free-XY/B#/A# 混合链，改为复制应用巡检已验证稳定的双通道方法论，但 Browser 与 Desktop 代码/状态彻底分离：① **可见文字**直接 patrol_browser_click_ocr_text(text=截图上真实文字)，与 desktop_click_ocr_text 一样由 Windows OCR boundingBox 决定精确中心，再用 Chrome Debugger trusted mouse 点击；② **无文字控件/小图标/空搜索框**先 patrol_observe(includeImage=true) 看完整 CURRENT browser screenshot，只给 patrol_browser_visual_action_map 一个粗区域 centerXRatio/centerYRatio，Browser 自己使用复制自 Desktop BuildActionMap 的边缘→双膨胀→连通区域→IoU 去重算法生成 V# bbox，read_image 看 V# 图后只把 candidateId=V# 交给 patrol_browser_click_visual_candidate；最终坐标只能由程序的 V# bbox center 产生。浏览器另有 wide-thin 控件兼容，用来保留搜索框这类宽而薄的矩形。旧 patrol_visual_click_target 的 A#/B#/preview/imageX/imageY/xRatio/yRatio 都只保留历史兼容，新 TEST 教学禁止使用。不要再做 focused crop→B#，不要让模型计算最终坐标，也不要拿 patrol-results 原始截图文件名猜路径。Browser Action Map 代码位于浏览器 runtime，自身保存 browser-vmap-* / V# 状态，绝不 import/call/修改 desktop-runtime 或 desktop_* visual state。
...
- 浏览器公开文本输入在普通 AUTO/HYBRID 模式仍可使用 patrol_type_text；但用户明确要求视觉点击时，搜索框/编辑框必须先由 patrol_browser_click_visual_candidate 或 patrol_browser_click_ocr_text 实际点击并验证 targetFocusedEditable=true，随后 PUBLIC 文本优先 patrol_type_focused_text(clear=true)，让输入跟随刚刚视觉命中的真实焦点，禁止重新用 selector 把文本送到另一个输入框。该工具禁止填写密码、OTP、token 或验证码。
...
- 在用户没有指定浏览器操作方式的默认 AUTO/HYBRID 策略下，TEST MODE 仍允许 browser_semantic_click / browser_click 等 semantic/selector CURRENT 页面恢复通道；但一旦用户明确指定视觉专用，现场点击统一使用 patrol_browser_click_ocr_text 或 patrol_browser_visual_action_map → patrol_browser_click_visual_candidate。patrol_visual_click_target、browser_visual_click 仅作为底层兼容/重放引擎，不再作为模型新教学入口。成功视觉教学仍把实际命中后的 learned semantic → learned selector → guarded visual geometry 保存给未来无人值守重放；这不改变当前教学轮次的纯视觉要求。
