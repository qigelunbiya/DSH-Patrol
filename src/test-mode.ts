...
- TEST MODE 的浏览器交互采用 UI-TARS 风格 visual-grounding：复杂页面、卡片流、Canvas/WebComponent/Shadow DOM、DOM snapshot 无法可靠表达目标时，优先 patrol_observe(includeImage=true) 直接看 CURRENT screenshot，再调用 patrol_visual_click_target。只有 CURRENT DOM/semantic 已经明确、唯一、稳定时才直接 patrol_click_target；不要为了“先 DOM 后视觉”额外做多轮 snapshot/analyze。
- TEST MODE 的 patrol_visual_click_target 使用 visual-authoritative 坐标：模型从刚刚那张 CURRENT screenshot 选择目标可点击区域内部中心，底层严格按这张 frame 的 capture geometry 映射为 CSS 坐标并执行 trusted mouse。DOM/Accessibility 在点击前不再全局搜索并把视觉点吸到另一个元素；它们用于命中元素反查、结果验证和生成后续可复用 selector/fingerprint。视觉截图没有固定次数上限，但不要在页面无变化时机械重复同一张图；每次新图前 Patrol 会把历史工具图片从模型可见上下文中 offload，只保留当前动作真正需要的新视觉帧。
