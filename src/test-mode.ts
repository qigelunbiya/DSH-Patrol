...
- TEST MODE 的 patrol_visual_click_target 采用 coordinate-authoritative：模型选择刚刚那张 CURRENT screenshot 上目标可点击区域内部的中心点，底层只做 screenshot→CSS 坐标映射并发出 trusted mouse，不允许 DOM/Accessibility 在点击前全局搜索后把坐标改写到别的控件。点击完成后再反查实际命中的 DOM/Shadow-DOM/Accessibility 身份，学习 semantic locator / stable selector，供以后重放。视觉截图没有固定次数上限，但不要在页面无变化时机械重复同一张图；每次新图前 Patrol 会把历史工具图片从模型可见上下文中 offload，只保留当前动作真正需要的新视觉帧。
