...
- TEST MODE 浏览器交互采用 UI-TARS 风格 visual-grounding：复杂页面、卡片流、Canvas、WebComponent、Shadow DOM、DOM snapshot 无法可靠表达目标，或视觉上明显比 DOM 更容易判断时，优先 patrol_observe(includeImage=true) 直接看 CURRENT screenshot，再调用 patrol_visual_click_target。只有 CURRENT DOM/semantic 已经明确、唯一、稳定时才直接 patrol_click_target；不要为了“先 DOM 后视觉”额外做多轮 snapshot/analyze。
- TEST MODE 的 patrol_visual_click_target 是 coordinate-authoritative：模型从刚刚那张 CURRENT screenshot 选择真实控件内部中心点，底层严格按该 frame 的 capture geometry 映射为 CSS 坐标并执行 trusted mouse。点击前 DOM/Accessibility 不得全局搜索并把视觉点吸附到另一个元素；DOM/Shadow DOM/Accessibility 只用于点击后的 hit-test、业务结果校验和 learned binding。视觉截图没有固定次数上限，但不要在页面无变化时机械重复同一张图；每次新图前 Patrol 会把历史工具图片从模型可见上下文中 offload，只保留当前动作真正需要的新视觉帧。
...
- TEST MODE 允许 browser_semantic_click / browser_click / browser_press / browser_scroll / browser_select / browser_navigate 作为 CURRENT 页面现场恢复通道。browser_visual_click 不作为模型可直接调用的逃生口；视觉点击统一通过 patrol_visual_click_target，使 frame 绑定、trusted input、点击后 DOM binding 和 Runbook 记录保持完整。视觉教学成功后，Runbook 保存 learned semantic locator / learned selector / fingerprint，并在重放时按 learned semantic → learned selector → guarded visual coordinate 的顺序执行，因此教学阶段可以 visual-grounding，而未来复用仍优先走稳定 DOM/语义命令。
...
- 如需确认模式调用 patrol_runtime_mode；TEST MODE 应报告 operational-click-fallbacks 且 browserControl=visual-grounding+dom-replay。需要恢复严格边界时设置 DSH_PATROL_CAPTCHA_MODE=normal 后彻底重启 Harness。`