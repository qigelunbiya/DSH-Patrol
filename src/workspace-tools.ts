...
export function registerPatrolWorkspaceTools(ctx: Context, store: PatrolStore): () => void {
  const paths = defineTool({
    name: 'patrol_paths',
    description: 'Show the absolute workspace-local Runbook and latest (or selected) run report/artifact paths. Call this before the final reply after creating, confirming, running, resuming, or summarizing a patrol.',
    parameters: {
