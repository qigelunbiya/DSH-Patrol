...
4. 不要直接调用 browser_*。普通教学优先使用 patrol_navigate、patrol_snapshot、patrol_read_page、patrol_count、patrol_login_state、patrol_refresh_image_code、patrol_click、patrol_press、patrol_scroll、patrol_wait、patrol_screenshot。兼容工具出现“tool arguments must be a JSON object”时停止重试该兼容调用并改用 flat Patrol 工具。仅在 normal captcha mode 且工具列表实际存在 patrol_detect_auth_challenge 时，才可用它分类非普通图片验证码。
...
8. 普通图片字符验证码 image-code 在当前默认 test mode 下走“紧凑截图 + 模型视觉 + 置信度门槛”：调用 browser_capture_image_code_visual 获取 CURRENT 验证码裁图，只给出一个最终识别值；置信度 >= 0.90 才调用 patrol_type_current_image_code。多个候选或置信度不足时调用 patrol_refresh_image_code 换图后重新抓取，最多 3 次，不要提交弱猜测。
9. image-code 永远不得写入 Runbook、notes、报告、checkpoint、总结或用户可见回复；不得调用 patrol_prepare_verification_handoff，不得 patrol_add_checkpoint，也不得询问用户验证码内容。patrol_screenshot 若返回 verification-suppressed 且 subtype=image-code，只表示通用整页 OCR 被抑制；使用 browser_capture_image_code_visual 获取当前裁图。
10. 不要在每次验证码或 detector 后无条件调用 patrol_prepare_verification_handoff。OTP/一次性动态码、设备确认、Passkey、二维码确认，以及第三方 reCAPTCHA/hCaptcha/Turnstile/Arkose 或其他明确不支持的验证可以人工暂停；普通 image-code 只能按当前裁图识别、置信度不足则刷新。
...
14. 需要截图时使用 patrol_screenshot；需要页面总结/周报的主要事实时优先 patrol_read_page。截图 OCR 只作为补充视觉文字。验证码图片识别优先使用 browser_capture_image_code_visual 的当前紧凑裁图，不要从历史截图或已输入文本复用验证码。
...
安全边界：Patrol runner 只允许固定 browser allowlist，browser_eval 和任意 browser tool 名称不被接受。用户直接提供的敏感值可以在 patrol_type_transient 这一个受控工具调用中短暂存在，以便首次填写与 AES-256-GCM 加密；持久化后 Runbook 只保存不透明引用，后续浏览器 provider 在执行内部解密，绝不把明文写进 Runbook/报告/工具卡片。普通 image-code 在默认 test mode 下走 CURRENT 紧凑裁图视觉识别和置信度门槛；第三方 CAPTCHA 和真正的 OTP/设备确认仍按人工 handoff 边界处理。`
