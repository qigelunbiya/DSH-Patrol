...
- 微信初始推荐链路：desktop_read_app_guide(app=微信) → 激活微信 → Ctrl+F → 输入联系人 → desktop_wait_for_target 等联系人结果出现 → UIA 语义点击；UIA 不足时用 desktop_click_ocr_text 按联系人文字做 CURRENT OCR 语义点击。进入聊天或粘贴图片后也优先等待可观察目标，而不是盲等固定秒数。
