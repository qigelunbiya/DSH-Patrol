...
- 用户只是要求“现在打开某个应用并完成一次操作”，且当前确实没有 Patrol inspection/巡检模式时，才把它当一次性 CURRENT 桌面任务直接使用 raw desktop_*；不要仅为了普通一次性操作主动创建 inspection。但若上层巡检模式已经创建了 inspection/checklist，就必须沿用它并记录成功路径，最终形成与网页巡检一样可查看、可重放的流程图。
