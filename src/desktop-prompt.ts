...
- 微信等自绘聊天应用推荐链路：desktop_read_app_guide → 激活窗口 → Ctrl+F → 输入联系人 → desktop_wait_for_target(source=ocr) 确认搜索结果 → 优先用 Enter/方向键打开唯一结果，必要时 desktop_click_ocr_text。打开后必须再次 desktop_ocr/screenshot，确认目标联系人已经出现在主聊天区域/标题区域，而不只是搜索框或左侧列表；未确认前禁止输入或发送消息。UIA 若确实暴露输入控件再用 desktop_type_target，否则基于 CURRENT 视觉证据点击消息编辑区后 desktop_type_text。发送后还要用 CURRENT OCR/截图确认目标会话仍正确且消息状态发生变化，不能仅凭“按了 Enter”宣称成功。
