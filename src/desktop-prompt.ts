...
- UIA 能唯一找到目标时优先 desktop_click_target；UIA 暴露不足时 desktop_ocr 获取 CURRENT 文字与每行绝对屏幕 rect/center。只有 OCR 文本唯一匹配目标时，才可直接使用该行返回的 center 作为 desktop_click_coordinates 的 CURRENT 坐标后备；OCR 结果不唯一时不要猜目标。
