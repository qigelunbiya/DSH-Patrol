...
describe('stable managed Patrol browser controller', () => {
  ...
  it('best-effort refreshes an extension that has legacy visualClick but lacks trustedVisualClick', async () => {
    const paths = fixture('dsh-patrol-stable-visual-refresh-')
      ...
      async evaluate() {
        ...
        state.extension = {
          ...
          capabilities: refreshes > 0
            ? ['captureImageCode', 'semanticClick', 'visualClick', 'trustedVisualClick', 'trustedFocusedType', 'trustedSemanticClick', 'clickOpenedTabAdoption', 'compactVisualCapture', 'boundedVisualCaptureV2', 'reusableVisualFramesV1', 'visualCoordinateGuideV1', 'visualPointerProbeV1', 'focusedVisualRegionV1']
    ...
    expect(state.extension?.capabilities).toContain('trustedVisualClick')
    await controller.dispose()
  })
  ...
  it('forces an in-place refresh when compactVisualCapture is present but boundedVisualCaptureV2 is missing', async () => {
    ...
    const worker = {
      async evaluate() {
        ...
        state.extension = {
          name: 'dsh-patrol-browser-extension',
          version: refreshes > 0 ? '0.3.10' : '0.3.6',
          capabilities: refreshes > 0
            ? [...currentCapabilities, 'boundedVisualCaptureV2', 'reusableVisualFramesV1', 'visualCoordinateGuideV1', 'visualPointerProbeV1', 'focusedVisualRegionV1']
            : currentCapabilities,
        }
      },
    }
  ...
  it('refreshes a 0.3.7-style extension that has bounded capture but still consumes visual frames', async () => {
    ...
    const worker = {
      async evaluate() {
        ...
        state.extension = {
          name: 'dsh-patrol-browser-extension',
          version: refreshes > 0 ? '0.3.10' : '0.3.7',
          capabilities: refreshes > 0
            ? ['captureImageCode', 'semanticClick', 'visualClick', 'trustedVisualClick', 'trustedFocusedType', 'trustedSemanticClick', 'clickOpenedTabAdoption', 'compactVisualCapture', 'boundedVisualCaptureV2', 'reusableVisualFramesV1', 'visualCoordinateGuideV1', 'visualPointerProbeV1', 'focusedVisualRegionV1']
    ...
    const worker = {
      async evaluate() {
        ...
        state.extension = {
          name: 'dsh-patrol-browser-extension',
          version: '0.3.1',
          capabilities: ['captureImageCode', 'semanticClick', 'boundedVisualCaptureV2', 'reusableVisualFramesV1', 'visualCoordinateGuideV1', 'visualPointerProbeV1', 'focusedVisualRegionV1'],
        }
      },
    }
    const extension = {
