...
describe('Patrol test-mode guard policy', () => {
  ...
  it('uses dedicated Windows OCR before model-visual fallback without re-reading an already known CURRENT code', () => {
    ...
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/同一张 CURRENT 验证码的唯一候选/)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/下一步直接调用 patrol_type_current_image_code/)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/不要为了形式上的 OCR 优先级又对同一张验证码调用 patrol_windows_ocr_image_code/)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/已有可靠 CURRENT 识别结果不得重复识别/)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/动态识别并填写图片验证码.*失败并停止/s)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/绝不能.*继续到.*填写动态口令/s)
