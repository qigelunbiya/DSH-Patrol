...
describe('Patrol test-mode guard policy', () => {
  ...
  it('uses Patrol local OCR before model vision and keeps test-mode click fallbacks operational', () => {
    ...
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/只用视觉模型.*visualAuthority=true/s)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/禁止视觉.*不得调用 patrol_observe\(includeImage=true\).*patrol_visual_click_target/s)
    ...
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/不得在点击前全局搜索后改写模型选中的坐标/)
