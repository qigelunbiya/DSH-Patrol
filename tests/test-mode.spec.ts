...
describe('Patrol test-mode guard policy', () => {
  ...
  it('uses Patrol local OCR before model vision and keeps test-mode click fallbacks operational', () => {
    ...
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/明确禁止视觉.*不得使用上述三个视觉入口.*includeImage=true/s)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/V# runtime\/state 与 Desktop 的 D# runtime\/state 完全隔离/)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/模型只选择文字或 V#/)
    expect(PATROL_TEST_MODE_OVERRIDE_PROMPT).toMatch(/最终点击坐标由程序计算/)
