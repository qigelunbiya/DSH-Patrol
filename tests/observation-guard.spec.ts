...
function execution(name: string, inspectionId: string, rootCallId: string) {
  return { name, arguments: { inspectionId }, rootCallId }
}
...
describe('current-state observation gate', () => {
  it('blocks browser mutation before a fresh observation', () => {
    ...
    expect(gate.guard(execution('patrol_navigate', 'demo', 'turn-1'))).toMatch(/patrol_observe/)
  })
