...
describe('Desktop Automation runtime foundation', () => {
  ...
  it('makes desktop application patrol visual-first and provides a safe window-relative visual click primitive', () => {
    ...
    expect(backend).toContain('Get-Process -ErrorAction SilentlyContinue')
    expect(backend).toContain("rectSource = 'process-hwnd-discovery'")
    expect(backend).toContain("title = ''")
    ...
    expect(backend).toContain('SendMessageTimeoutW')
    expect(backend).toContain('SMTO_ABORTIFHUNG')
    expect(backend).toContain('80,')
    expect(backend).toContain('GetProcessName')
    expect(backend).toContain('GetVisibleTopLevelWindowRecords(1200)')
