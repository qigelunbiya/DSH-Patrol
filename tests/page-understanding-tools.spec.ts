...
describe('Patrol page understanding planner', () => {
  ...
  it('keeps image-code on its dedicated OCR/token-gated path in TEST MODE', () => {
    ...
    expect(PATROL_PAGE_UNDERSTANDING_PROMPT).toMatch(/pixelActionMap=true/)
    expect(PATROL_PAGE_UNDERSTANDING_PROMPT).toMatch(/pixelCandidateId=B#/)
    expect(PATROL_PAGE_UNDERSTANDING_PROMPT).toMatch(/DOM\/Accessibility 不参与 B# 生成/)
  })
})
