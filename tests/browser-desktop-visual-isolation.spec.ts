...
describe('browser/desktop visual plane isolation', () => {
  ...
  it('keeps desktop visual runtime independent from browser visual frames and browser click primitives', () => {
    ...
    expect(desktopSources).not.toMatch(/browser-vmap-/)
    expect(desktopSources).not.toMatch(/patrol_browser_visual_action_map/)
    expect(desktopSources).not.toMatch(/patrol_browser_click_visual_candidate/)
    expect(desktopSources).not.toMatch(/interactionBrowserVisualActionMaps/)
  })
})
