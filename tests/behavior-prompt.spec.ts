...
describe('current Patrol behavior prompt', () => {
  ...
  it('uses the copied Desktop workflow through browser-only OCR and V# tools', () => {
    ...
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/V1\/V2\/\.\.\./)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/Desktop BuildActionMap/)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/threshold=30/)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/双 dilation/)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/IoU=0\.42/)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/wide-thin rectangle/)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/模型永远不提供最终 x\/y/)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/旧 B#\/A#\/focused crop\/free imageX\/imageY\/xRatio\/yRatio/)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/Browser 与 Desktop 不共享 runtime、frame、candidate、map/)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/trusted Chrome debugger mouse/)
