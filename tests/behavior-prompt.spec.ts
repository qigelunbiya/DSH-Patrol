...
describe('current Patrol behavior prompt', () => {
  ...
  it('uses vision-first teaching with post-click DOM learning and Desktop-style exact visual ratios', () => {
    ...
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/actionMap=true, targetHint=完整行身份\+动作.*ACTION MAP TARGET ZOOM.*mark.*previewId/s)
    expect(PATROL_BEHAVIOR_PROMPT).toMatch(/TEST MODE.*不允许第一次估点就直接执行物理左键.*pointerAction=mark.*previewId/s)
